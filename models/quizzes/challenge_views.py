...
@bp.route('/challenge/<int:student_id>/', methods=['GET', 'POST'])
@requires_access_level(UserConstants.USER_TYPES['USER'])
def do_challenge(student_id):
    form = ChallengeForm(request.form)
    challenged_user = User.query.get(student_id)
    if student_id == g.user.id:
    ...
    if form.validate_on_submit():
        try:
            ...
            return redirect(url_for('.do_challenge', student_id=student_id, warn=e.message))
...
def check_question(challenge_id):
    ...
    try:
        ...
        challenge_attempt = ChallengeAttempt.query.filter(ChallengeAttempt.student_id == g.user.id,
